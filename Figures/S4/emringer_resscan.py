...
def main(args):
	for file in args.filenames:
		...
		# easy_pickle.dump(file+'.adjusted_output/Outliers.pkl',Weird_residues)
		# #print 'Wrote ' + file+'.adjusted_output/Outliers.pkl'
		# easy_pickle.dump(file+'.adjusted_output/rotamer_ratios.pkl', rotamer_ratios)
		# #print 'Wrote ' + file+'.adjusted_output/rotamer_ratios.pkl'
		# easy_pickle.dump(file+'.adjusted_output/zscores.pkl', zscores)
		# #print 'Wrote ' + file+'.adjusted_output/zscores.pkl'
		# easy_pickle.dump(file+'.adjusted_output/thresholds.pkl', thresholds)
		# #print 'Wrote ' + file+'.adjusted_output/thresholds.pkl'
		# easy_pickle.dump(file+'.adjusted_output/peak_counts.pkl', peak_count)
		# #print 'Wrote ' + file+'.adjusted_output/peak_counts.pkl'
		...
		# 		#print "Absolute EMRinger Score: %.3f" % float(value)
		# 		#print "Model Length: %d" % length
		# 		# #print "Z-score/(length): %.8f" % (value/length)
		# 		#print "Adjusted EMRinger Score: %8f" % (10*value/math.sqrt(length))
		# 		break
		print "EMRinger Score for %s: %f" %(file,10*max(zscores)/math.sqrt(length))
...
# 	plt.savefig('%s.adjusted_output/%.3f.Phenixed_Histogram.png' % (filename,threshold))
...
# 	plt.title('Peak Counts - Threshold %.3f' % (threshold), y=1.05)
# 	plt.xticks([i*60 for i in range(7)])
# 	plt.xlim(0,360)
# 	plt.xlabel(r'Chi1 Angle ($\degree$)', labelpad=10)
# 	plt.ylabel("Peak Count", labelpad=10)
# 	plt.savefig('%s.adjusted_output/%.3f.Phenix_allpeaks.png' % (filename,threshold))
# 	#print 'Saved plot to %s.adjusted_output/%.3f.Phenix_allpeaks.png' % (filename,threshold)
...
# 	ax1.set_ylabel('Adjsuted EMRinger Score', color='b')
# 	for tl in ax1.get_yticklabels():
# 		tl.set_color('b')
# 	ax1.set_ylim([-4,4])
# 	ax1.axhspan(-1,1,color='b',alpha=0.1)
# 	ax2 = ax1.twinx()
# 	ax2.grid()
# 	ax2.plot(non_zero_thresholds, rotamer_ratios, 'r-', label = i, linewidth=3.0, alpha=0.7)
# 	ax2.set_ylim([float(1.0/12),1])
...
# 	plt.savefig('%s.adjusted_output/%s.threshold_scan.png' % (file, i))
# 	#print 'Saved plot to %s.adjusted_output/%s.threshold_scan.png' % (file, i)
